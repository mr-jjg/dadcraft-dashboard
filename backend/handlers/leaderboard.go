...
SELECT c.name,
       ...
       (c.totaltime - c.leveltime) AS time_played
...
const frontierOrder = `ORDER BY c.level DESC, d.dinged_at ASC, time_played ASC`
const speedrunOrder = `ORDER BY c.level DESC, time_played ASC, d.dinged_at ASC`

func GetLeaderboard(dbRepo repository.DBRepository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		order := frontierOrder
		if r.URL.Query().Get("sort") == "speedrun" {
			order = speedrunOrder
		}

		tableResult, err := dbRepo.QueryDatabase(leaderboardQuery + order)
		if err != nil {
		...
		for _, row := range tableResult.Rows {
			// columns: name(0), level(1), race(2), class(3), online(4), ding_time(5), time_played(6)
			level, _ := strconv.Atoi(row[1])
			dingTime, _ := strconv.ParseInt(row[5], 10, 64)
			timePlayed, _ := strconv.ParseInt(row[6], 10, 64)
