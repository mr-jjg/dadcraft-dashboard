...
func main() {
	...
	mux.HandleFunc("/api/system/uptime", handlers.GetMetric(repo, `time() - node_boot_time_seconds`))
	mux.HandleFunc("/api/system/load1", handlers.GetMetric(repo, `node_load1`))
	mux.HandleFunc("/api/system/load5", handlers.GetMetric(repo, `node_load5`))
	mux.HandleFunc("/api/system/load15", handlers.GetMetric(repo, `node_load15`))
	mux.HandleFunc("/api/system/cpu", handlers.GetMetric(repo, `100 - (avg by (instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)`))
	mux.HandleFunc("/api/system/memory", handlers.GetMetric(repo, `(1 - (node_memory_MemAvailable_bytes / node_memory_MemTotal_bytes)) * 100`))
	mux.HandleFunc("/api/system/swap", handlers.GetMetric(repo, `(1 - (node_memory_SwapFree_bytes / node_memory_SwapTotal_bytes)) * 100`))
	mux.HandleFunc("/api/system/disk", handlers.GetMetric(repo, `(1 - (node_filesystem_avail_bytes{mountpoint="`+diskMountpoint+`"} / node_filesystem_size_bytes{mountpoint="`+diskMountpoint+`"})) * 100`))
	mux.HandleFunc("/api/system/io", handlers.GetMetric(repo, `avg by (instance) (rate(node_cpu_seconds_total{mode="iowait"}[5m])) * 100`))
	mux.HandleFunc("/api/system/rx", handlers.GetMetric(repo, `rate(node_network_receive_bytes_total{device="`+networkDevice+`"}[5m])`))
	mux.HandleFunc("/api/system/tx", handlers.GetMetric(repo, `rate(node_network_transmit_bytes_total{device="`+networkDevice+`"}[5m])`))
	mux.HandleFunc("/api/mangosd/uptime", handlers.GetMetric(repo, `time() - namedprocess_namegroup_oldest_start_time_seconds{groupname="mangosd"}`))
	...
	mux.HandleFunc("/api/realmd/uptime", handlers.GetMetric(repo, `time() - namedprocess_namegroup_oldest_start_time_seconds{groupname="realmd"}`))
	...
	mux.HandleFunc("/api/mysqld/uptime", handlers.GetMetric(repo, `time() - namedprocess_namegroup_oldest_start_time_seconds{groupname="mysqld"}`))
