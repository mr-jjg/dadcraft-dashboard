...
func main() {
	...
	mux.HandleFunc("/api/system/cpu/range",  handlers.GetMetricRange(promRepo, `100 - (avg by (instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)`))
	...
	mux.HandleFunc("/api/system/swap/range", handlers.GetMetricRange(promRepo, `(1 - (node_memory_SwapFree_bytes / node_memory_SwapTotal_bytes)) * 100`))
	...
	mux.HandleFunc("/api/system/disk/range", handlers.GetMetricRange(promRepo, `(1 - (node_filesystem_avail_bytes{mountpoint="`+diskMountpoint+`"} / node_filesystem_size_bytes{mountpoint="`+diskMountpoint+`"})) * 100`))
	mux.HandleFunc("/api/system/io", handlers.GetMetric(promRepo, `avg by (instance) (rate(node_cpu_seconds_total{mode="iowait"}[5m])) * 100`))
	mux.HandleFunc("/api/system/io/range",   handlers.GetMetricRange(promRepo, `avg by (instance) (rate(node_cpu_seconds_total{mode="iowait"}[5m])) * 100`))
	mux.HandleFunc("/api/system/rx", handlers.GetMetric(promRepo, `rate(node_network_receive_bytes_total{device="`+networkDevice+`"}[5m])`))
	mux.HandleFunc("/api/system/rx/range",   handlers.GetMetricRange(promRepo, `rate(node_network_receive_bytes_total{device="`+networkDevice+`"}[5m])`))
	...
	mux.HandleFunc("/api/progression", handlers.GetProgression(dbRepo))
	mux.HandleFunc("/api/progression/timestamps", handlers.GetProgressionTimestamps(dbRepo))
