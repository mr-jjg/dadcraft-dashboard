...
INSERT INTO dadcraft_dashboard.progression_snapshots (scrape_id, level, race, class, online, guild, count)
...
LEFT JOIN v_characters.guild_member gm ON c.guid = gm.guid
LEFT JOIN v_characters.guild g ON gm.guildid = g.guildid
WHERE a.gmlevel = 0
GROUP BY c.level, c.race, c.class, c.online, g.name;
