...
const defaultPageProps = { pageSize: 25, onPageSizeChange: vi.fn() }

test('renders nothing when table is null', () => {
    const { container } = render(<TableView table={null} {...defaultPageProps} />)
    expect(container.firstChild).toBeNull()
})

test('renders column headers', () => {
    render(<TableView table={{ columns: ['Name', 'Level', 'Race'], rows: [] }} {...defaultPageProps} />)
...
test('renders mapped column label when available', () => {
    render(<TableView table={{ columns: ['in_battleground'], rows: [] }} {...defaultPageProps} />)
    expect(screen.getByText('In BG')).toBeInTheDocument()
})

test('renders raw column name when no mapping exists', () => {
    render(<TableView table={{ columns: ['custom_column'], rows: [] }} {...defaultPageProps} />)
    expect(screen.getByText('custom_column')).toBeInTheDocument()
})

test('formats money column as gold silver copper', () => {
    render(<TableView table={{ columns: ['money'], rows: [['1089820']] }} {...defaultPageProps} />)
...
test('renders rows', () => {
    render(<TableView table={SIMPLE_TABLE} {...defaultPageProps} />)
...
test('renders empty tbody when rows is empty', () => {
    render(<TableView table={{ columns: ['Name'], rows: [] }} {...defaultPageProps} />)
    expect(screen.getByRole('table')).toBeInTheDocument()
    expect(screen.queryAllByRole('row').length).toBe(1)
})

test('renders each cell in a row', () => {
    render(<TableView table={{ columns: ['a', 'b', 'c'], rows: [['x', 'y', 'z']] }} {...defaultPageProps} />)
...
test('does not mutate original rows', () => {
    ...
    render(<TableView table={table} {...defaultPageProps} />)
    fireEvent.click(screen.getByText('Name'))
...
test('renders Prev and Next buttons', () => {
    render(<TableView table={LARGE_TABLE} {...defaultPageProps} />)
...
test('changing page size resets to page 1', () => {
    ...
    render(<TableView table={LARGE_TABLE} pageSize={25} onPageSizeChange={onPageSizeChange} />)
    fireEvent.click(screen.getByRole('button', { name: 'Next' }))
    expect(getPageIndicator('26 - 30 of 30')).toBeInTheDocument()
...
test('unchecking All columns collapses to searched fields', () => {
    ...
    render(<TableView table={SIMPLE_TABLE} searchedFields={searchedFields} {...defaultPageProps} />)
    fireEvent.click(screen.getByRole('button', { name: 'Columns' }))
...
test('column panel closes when clicking outside', async () => {
    render(
        <div>
            <TableView table={SIMPLE_TABLE} {...defaultPageProps} />
...
test('respects initialVisibleCols prop', () => {
    render(<TableView table={SIMPLE_TABLE} initialVisibleCols={['Name']} {...defaultPageProps} />)
    expect(screen.getByRole('columnheader', { name: /Name/ })).toBeInTheDocument()
    expect(screen.queryByRole('columnheader', { name: /Level/ })).not.toBeInTheDocument()
})
